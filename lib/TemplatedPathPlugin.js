...
const replacePathVariables = (path, data) => {
	...
	// [file] - data.filename (/context/file.map.js)
	// [filebase] - data.basename (file.map.js)
	if (data.chunk) {
		...
		const chunkHashWithLength =
			"hashWithLength" in chunk && chunk.hashWithLength;
		...
		const contentHashWithLength =
			data.contentHashWithLength || "contentHashWithLength" in chunk
		...
		return (
			path
				...
				.replace(REGEXP_QUERY, replacer(data.query, true))
				.replace(REGEXP_FILE, replacer(data.filename))
				.replace(REGEXP_FILEBASE, replacer(data.basename))
			// TODO
			// .replace(REGEXP_EXT, replacer(extensions[type]))
		);
	}
	...
	//
	// [id] - module.id (2.png)
	// [ext] - extname(module.resource || data.filenam) (file.{png, svg, ...})
	// [hash] - module.hash (6237543873.png)
	// [name] - file.replace(extname, '') (file)
	// [file] - basename(module.resource || data.filename) (file.png)
	//
	// Legacy Placeholders
	//
	...
	if (data.module) {
		...
		const hashWithLength = "hashWithLength" in module && module.hashWithLength;

		const file = basename(data.filename || "");
		...
		return (
			path
				...
				.replace(REGEXP_EXT, replacer(ext))
				.replace(REGEXP_NAME, replacer(name))
				.replace(REGEXP_FILE, replacer(file))
